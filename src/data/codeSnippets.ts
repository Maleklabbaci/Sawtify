...
│   │   │   ├── 📄 StyleTagsBar.tsx    # Insertion rapide de balises (<laugh>, <sigh>, etc.)
...
CREATE TABLE IF NOT EXISTS public.generations (
    ...
    style_tags TEXT[],                 -- Balises audio appliquées: ['<laugh>', '<sigh>', '<short pause>']
...
class TTSGenerationRequest(BaseModel):
    ...
    # La voix accepte 3 écritures : prénom français ("Amine"), prénom arabe ("أمين"),
    # identifiant technique ("Puck") ou identifiant historique ("voice_amin").
    voice_id: str = Field(default="voice_amin", description="ID de la voix")
...
// Extrait du catalogue : Sawtify compte 30 voix, toutes multilingues (la langue
// parlée est détectée automatiquement, ce n'est pas un critère de choix).
// Liste complète et à jour : GET /api/v1/tts/voices
const VOICES: Voice[] = [
  { id: "voice_amin",   name: "Amine",   caractere: "Voix enjouée et dynamique",  gender: "Homme" }, // Puck
  { id: "voice_khalid", name: "Khalid",  caractere: "Voix informative et posée",   gender: "Homme" }, // Charon
  { id: "voice_rashid", name: "Rachid",  caractere: "Voix énergique et survoltée", gender: "Homme" }, // Fenrir
  { id: "voice_yasmin", name: "Yasmine", caractere: "Voix éclatante et souriante", gender: "Femme" }, // Zephyr
  { id: "voice_maryam", name: "Maryam",  caractere: "Voix chaleureuse et douce",   gender: "Femme" }, // Sulafat
  { id: "voice_nour",   name: "Nour",    caractere: "Voix douce et apaisante",     gender: "Femme" }, // Achernar
];

// Balises de SONS (chevrons). Elles s'écrivent aussi en français et en arabe :
// "<laugh>" = "<rire>" = "<ضحكة>". La vitesse et le ton général ne sont PAS des
// balises : ils se règlent avec les paramètres speed / pitch (ou le style).
const STYLE_TAGS = ["<laugh>", "<sigh>", "<gasp>", "<whispers>", "<breath>", "<short pause>"];

export default function TTSGenerationDashboard() {
  const [text, setText] = useState("");
  const [selectedVoice, setSelectedVoice] = useState("voice_amin");
  ...
  // Insérer une balise de son à la position courante du curseur
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 space-y-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {VOICES.map(voice => (
            ...
            >
              ...
              <span className="block text-xs text-gray-500 mt-1">{voice.caractere}</span>
            </button>
          ))}
        </div>
      </div>
      ...
      <div>
        <span className="block text-xs font-medium text-gray-500 mb-1.5">Sons et émotions (balises) :</span>
      ...
      <div className="space-y-2">
        <textarea
          ...
          rows={5}
          placeholder="Écrivez ou collez votre texte ici... Utilisez les balises <laugh> ou <sigh> pour ajouter un son, en français (<rire>) ou en arabe (<ضحكة>)."
