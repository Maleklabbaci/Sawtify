  ...
  return <div className="mx-auto w-full max-w-6xl space-y-6 animate-in fade-in">
    ...
    <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
      ...
      <div className="space-y-4">
        <div><div className="mb-2 text-xs font-black text-slate-700">Police — aperçu réel</div><div className="grid grid-cols-2 gap-2 sm:grid-cols-4 lg:grid-cols-5">{CAPTION_FONTS.map((font, index) => <button key={font} type="button" onClick={() => setCaptionFont(font)} className={`rounded-xl border-2 p-2 text-left transition ${captionFont === font ? 'border-purple-600 bg-purple-50 ring-2 ring-purple-100' : 'border-slate-200 hover:border-purple-300'}`}><span className="block truncate text-xs font-bold text-slate-500">{font}</span><span className="mt-1 block truncate text-base font-bold text-slate-900" style={{ fontFamily: index < 10 ? 'Arial' : 'sans-serif' }}>واش راك؟</span><span className="block truncate text-[10px] text-slate-500">Montage vidéo</span></button>)}</div></div>
