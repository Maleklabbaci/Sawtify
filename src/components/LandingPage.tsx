...
import { Helmet } from "react-helmet-async"; // 🔥 SEO AJOUTÉ
import {
  ArrowRight,
  ArrowLeft,
  Play,
...
const MeshBackground = () => {
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      let changed = false;
      for (let i = 0; i < 6; i++)
        if (Math.abs(out[i] - applied[i]) > 0.25) {
          changed = true;
          break;
        }
      if (changed) {
  ...
  return (
    <div className="flex items-center gap-2.5 select-none">
      ...
      <span
        ...
        style={{ color: INK, fontFamily: NUM_STACK }}
      >
        Sawtify
      </span>
    </div>
...
const Num = ({
  ...
  <span
    dir="ltr"
    style={{ unicodeBidi: "isolate", fontFamily: NUM_STACK }}
...
const Mono = ({
  ...
  <span
    dir={dir}
    className={className}
    style={{ fontFamily: MONO_STACK, ...style }}
  >
    {children}
  </span>
);
const SectionHead = ({
  ...
  <div className={center ? "text-center mx-auto max-w-2xl" : "max-w-2xl"}>
    <h2
      className="text-[clamp(1.9rem,4.2vw,3.1rem)] leading-[1.06] tracking-[-0.02em] font-bold"
      style={{ color: INK, fontFamily: font }}
    >
      {title}
    </h2>
    {sub && (
      <p className="mt-4 text-[14px] text-[#16121F]/60 leading-relaxed">
        {sub}
      </p>
    )}
  </div>
...
function useSampleAudio() {
  ...
  const playSample = useCallback(
    (voiceId: string, audioUrl?: string) => {
      if (!audioUrl) return;
      const a = ensureAudio();
      a.pause();
...
const AudioDock = ({
  ...
  return (
    ...
    >
      <div
        className="flex items-center gap-3 rounded-full border bg-white pl-4 pr-1.5 py-1.5 shadow-[0_10px_36px_rgba(22,18,31,0.16)]"
      ...
      >
        ...
        <div
          className="flex items-end gap-[2.5px] h-4 shrink-0"
          dir="ltr"
          aria-hidden
        >
          {F.map((f, i) => (
        ...
        <span
          ...
          style={{
            ...
            color: isPlaying ? ACCENT : "rgba(22,18,31,0.55)",
          }}
        >
          {isPlaying
            ? isRTL
...
const VOICES: VoiceCard[] = [
  {
    ...
    sampleAr:
      "سلام عليكم خاوتي! مع صوتيفي، نصوصكم تولي صوت طبيعي، واضح، جاهز للإعلانات.",
    audioUrl:
  ...
  },
  {
    ...
    color: "#DB2777",
    sampleFr:
      "Marhba bikom kamlin! Tawsil 58 wilaya, payment 3and l istlam. Tleb dorka.",
    sampleAr:
      "مرحبا بيكم كاملين! التوصيل لـ 58 ولاية والدفع عند الاستلام. اطلب درك.",
    audioUrl:
  ...
  },
  {
    ...
    color: "#D97706",
    sampleFr:
      "Salut l'équipe ! Une voix vive, parfaite pour Reels, TikTok et stories.",
  ...
  },
  {
    ...
    color: "#0891B2",
    sampleFr:
      "Dans cette leçon, on avance pas à pas. Une voix claire, pour e-learning et tutos.",
  ...
  },
  {
    ...
    color: "#059669",
    sampleFr:
      "Bienvenue dans cet épisode. Une voix chaleureuse, pour podcasts et YouTube.",
  ...
  },
  {
    ...
    color: "#BE185D",
    sampleFr:
      "Écoutez une diction fluide et élégante, pour vos récits et documentaires.",
  ...
  },
  {
    ...
    color: "#EA580C",
    sampleFr:
      "Une voix percutante, idéale pour vos spots et lancements produits.",
  ...
  },
  {
    ...
    color: "#0284C7",
    sampleFr:
      "Le rendu est si naturel qu'on croirait un présentateur en studio.",
...
const LANDING_VOICES = VOICES.filter((v) =>
  (LANDING_VOICE_IDS as readonly string[]).includes(v.id),
);
const HIDDEN_VOICES = VOICES.filter(
  (v) => !(LANDING_VOICE_IDS as readonly string[]).includes(v.id),
);
const COST_STEPS = [
...
export const LandingPage: React.FC<LandingPageProps> = ({
  ...
  const readLevel = useCallback((): number => {
    ...
    return Math.min(
      1,
      Math.abs(Math.sin(t) * Math.cos(t * 0.7)) * 0.8 + Math.random() * 0.2,
    );
  }, []);
  ...
  const t = {
    ...
    welcomeChip: isRTL
      ? "50 نقطة مجاناً. بدون بطاقة."
      : "50 points offerts. Sans carte.",
    ...
    journeyTitle: isRTL
      ? "أربع خطوات. يخرج الصوت."
      : "Quatre gestes. La voix sort.",
    journeySub: isRTL
      ? "بدون كابينة. بدون ميكروفون. بدون انتظار."
      : "Pas de cabine. Pas de micro. Pas d'attente.",
    useTitle: isRTL
      ? "حين يتكلم، لم يعد نصًا."
      : "Quand ça parle, ce n'est plus du texte.",
  ...
  const dispElapsed = isIntroPlaying
    ? playElapsed
    : heroSamplePlaying
      ? sampleElapsed
      : 0;
  const dispTotal = isIntroPlaying
    ? playTotal
    : heroSamplePlaying
      ? sampleTotal
      : durations[featured.id] || 0;
  const dispProgress = isIntroPlaying
    ? playProgress
    : heroSamplePlaying
      ? sampleProgress
      : 0;
  ...
  const journeySteps = [
    {
      ...
      t: isRTL ? "اكتب" : "Écrire",
      d: isRTL
        ? "ألصق نصك بالدارجة، بالعربية أو بالفرنسية."
        : "Collez votre texte en darija, en arabe ou en français.",
    },
    {
      n: "2",
      t: isRTL ? "اختر" : "Choisir",
      d: isRTL
        ? "9 أصوات. هنا نعرض 3 فقط."
        : "9 voix. Ici, on n'en montre que 3.",
    },
    {
      n: "3",
      t: isRTL ? "اضبط" : "Régler",
      d: isRTL
        ? "السرعة، النبرة، التأثيرات… الباقي في الاستوديو."
        : "Vitesse, timbre, effets… le reste est dans le studio.",
    },
    {
      n: "4",
      t: isRTL ? "حمّل" : "Télécharger",
      d: isRTL
        ? "MP3 أو WAV. بلا علامة مائية. استعمال تجاري."
        : "MP3 ou WAV. Sans filigrane. Usage commercial.",
    },
  ];
  const uses = [
    {
      ...
      d: isRTL
        ? "سبوت، عرض، توصيل 58 ولاية."
        : "Spots, promos, livraison 58 wilayas.",
    },
    {
      icon: Clapperboard,
      t: isRTL ? "ريلز وتيك توك" : "Reels & TikTok",
      d: isRTL
        ? "صوت قصير وحيوي، جاهز للقصص."
        : "Voix courte, vive, prête pour les stories.",
    },
    {
      icon: Mic2,
      t: isRTL ? "بودكاست ويوتيوب" : "Podcast & YouTube",
      d: isRTL ? "سرد طويل، نبرة ثابتة." : "Narration longue, timbre stable.",
    },
    {
      icon: Phone,
      t: isRTL ? "موزّع هاتفي" : "Standard",
      d: isRTL
        ? "مرحبًا، اضغط 1، خدمة الزبائن."
        : "Bienvenue, tapez 1, service client.",
    },
  ];
  const metrics = [
  ...
  const testimonials = isRTL
    ? [
        {
          q: "جرّبت 5 منصات قبل صوتيفي. هنا الصوت يبدو بشريًا فعلًا. الزبائن لا يلاحظون الفرق.",
          n: "أمين ب.",
          r: "صانع محتوى، الجزائر",
          img: "AB",
        },
        {
          q: "استخدمته لإعلانات تجارية. نتيجة احترافية دون الحاجة إلى استوديو.",
          n: "ياسمين ق.",
          r: "وكالة إشهار، وهران",
          img: "YK",
        },
        {
          q: "أفضل صوت جزائري سمعته. طبيعي 100٪ والدفع بالذهبية مريح.",
          n: "خالد م.",
    ...
    : [
        {
          q: "J'ai testé 5 plateformes avant Sawtify. Ici, la voix sonne vraiment humaine. Mes clients ne font pas la différence.",
          n: "Amine B.",
          r: "Créateur, Alger",
          img: "AB",
        },
        ...
        {
          q: "La meilleure voix algérienne que j'ai entendue. Naturelle à 100 %, et le paiement Edahabia est simple.",
          n: "Khaled M.",
  ...
  const pricing = [
    {
      ...
      ptsLabel: "100",
      price: "500",
      desc: isRTL ? "للتجربة المرنة والحرة." : "Pour découvrir la plateforme.",
    },
    {
      pts: 220,
      ptsLabel: "220",
    ...
    },
    {
      ...
      price: "2 500",
      desc: isRTL
        ? "لمن يعمل يوميًا — وكالات وصنّاع محتوى."
        : "Pour un usage régulier — agences et créateurs.",
    },
    {
      pts: 1350,
      ptsLabel: "1 350",
      ...
      desc: isRTL
        ? "للمحترفين — حجم كبير."
        : "Pour les professionnels — grands volumes.",
    },
  ];
  const faqs = isRTL
    ? [
        {
          q: "هل الصوت يبدو كإنسان؟",
          a: "نعم. دارجة حيّة، 24 kHz. 99٪ ممن يسمعون لا يفرّقون.",
        },
        {
          q: "هل يمكن استعماله في الإعلان؟",
          a: "نعم. إعلان، يوتيوب، تيك توك، موزّع — استعمال تجاري كامل، بلا علامة مائية.",
        },
        {
          q: "كيف تعمل النقاط؟",
          a: "20 نقطة لـ 0–60 ثانية، ثم +10 لكل دقيقة. لا تنتهي. 50 نقطة مجاناً عند التسجيل.",
        },
        {
          q: "الذهبية و CIB؟",
          a: "نعم، SATIM، بالدينار. لا تحتاج بطاقة أجنبية.",
        },
        {
          q: "هل أجرّب مجانًا؟",
          a: "نعم. 50 نقطة مجاناً، بدون بطاقة. وهنا 3 أصوات فقط — الباقي في الاستوديو.",
        },
      ]
    : [
        {
          q: "La voix parle comme quelqu'un ?",
          a: "Oui. Darija vivante, 24 kHz. 99 % de ceux qui écoutent ne font pas la différence.",
        },
        {
          q: "Puis-je l'utiliser en pub ?",
          a: "Oui. Pub, YouTube, TikTok, standard — usage commercial, sans filigrane.",
        },
        {
          q: "Comment marchent les points ?",
          a: "20 points pour 0–60 s, puis +10 par minute. Ils n'expirent pas. 50 points offerts à l'inscription.",
        },
        {
          q: "Edahabia et CIB ?",
          a: "Oui, SATIM, en dinars. Pas besoin de carte étrangère.",
        },
        {
          q: "Je peux essayer sans payer ?",
          a: "Oui. 50 points offerts, sans carte. Ici seulement 3 voix — les autres sont dans le studio.",
        },
      ];
  const trust = [
  ...
  return (
    ...
    >
      ...
      <Helmet>
        {isRTL ? (
          <>
            ...
            <meta
              name="description"
              content="سوتيفاي: أول منصة تحويل النص إلى صوت متخصصة في الدارجة الجزائرية. دفع محلي CIB و Edahabia. 9 أصوات IA."
            />
            <html lang="ar" dir="rtl" />
            <meta
              property="og:title"
              content="صوتيفي — صوت IA بالدارجة الجزائرية"
            />
            <meta
              property="og:description"
              content="حوّل نصك بالدارجة إلى صوت طبيعي في 30 ثانية. دفع محلي CIB و Edahabia."
            />
          ...
          <>
            ...
            <meta
              name="description"
              content="Premier générateur de voix IA en darija algérienne. Convertissez texte en voix naturelle, paiement CIB & Edahabia. 9 voix IA."
            />
            <html lang="fr" dir="ltr" />
            <meta
              property="og:title"
              content="Sawtify — Voix Off Darija Algérienne"
            />
            <meta
              property="og:description"
              content="Premier générateur de voix IA en darija algérienne. Texte en voix naturelle, paiement CIB & Edahabia."
            />
        ...
        <link
          rel="alternate"
          hreflang="ar"
          href="https://sawtify.space/?lang=ar"
        />
        <link
          rel="alternate"
          hreflang="x-default"
          href="https://sawtify.space/"
        />
      </Helmet>
      ...
      <header
        className={`fixed top-0 inset-x-0 z-50 transition-all duration-300 ${scrolled ? "bg-white/85 backdrop-blur-xl border-b" : "bg-transparent"}`}
            ...
            >
              ...
              >
                ...
                <button
                  type="button"
                  onClick={() => setMenuOpen(false)}
                  className="w-10 h-10 rounded-full hover:bg-[#16121F]/5 flex items-center justify-center focus-ring"
                  aria-label={t.close}
                >
                  <X className="w-5 h-5" />
                </button>
              ...
              <nav className="flex-1 px-5 py-6 flex flex-col">
                ...
                <button
                  ...
                  className="mt-4 py-3 text-start text-[16px] font-semibold text-[#16121F]/60"
                >
                  {t.signin}
                </button>
              </nav>
      ...
      <div className="relative z-[1]">
        <section id="home" className="relative pt-32 pb-16 sm:pb-20">
          <div className="mx-auto max-w-[1280px] px-5 sm:px-6">
            <div className="max-w-3xl mx-auto text-center">
              ...
              <SlideUp delay={0.06}>
                <h1
                  className="text-[clamp(2.8rem,7vw,5.4rem)] leading-[0.98] tracking-[-0.03em] font-bold"
              ...
              <SlideUp delay={0.22}>
                <div className="mt-8 flex items-center justify-center gap-3 flex-wrap">
                  ...
                  <button
                    type="button"
                    onClick={handleToggleIntroAudio}
                  ...
                  >
                    ...
                    >
                      {isIntroPlaying ? (
                        ...
                        <Play className="w-3 h-3 fill-current ms-0.5" />
                      )}
                    </span>
                    {isIntroPlaying
                      ? isRTL
                ...
                <p
                  className="mt-4 text-[12px] font-semibold flex items-center justify-center gap-1.5"
                  style={{ color: ACCENT }}
                >
            ...
            <SlideUp delay={0.28}>
              <section className="py-6 sm:py-8 px-4" aria-label="Définition">
                <p className="text-[14px] sm:text-[15px] text-[#16121F]/70 leading-relaxed max-w-2xl mx-auto text-center">
                  {isRTL ? (
                    <>
                      <strong className="font-bold text-[#16121F]">
                        سوتيفاي
                      </strong>{" "}
                      هو أول مولّد أصوات بالذكاء الاصطناعي متخصص في الدارجة
                      الجزائرية. يحوّل أي نص مكتوب — بالعربية أو بالحروف
                      اللاتينية — إلى صوت طبيعي في أقل من 30 ثانية، مباشرة من
                      المتصفح، بدون تثبيت. نظام نقاط بالدفع المحلي (CIB و
                      Edahabia).
                    </>
                  ) : (
                    <>
                      ...
                      est le premier générateur de voix IA spécialisé en darija
                      algérienne. Il transforme n'importe quel texte — en arabe
                      ou en alphabet latin — en voix naturelle en moins de 30
                      secondes, directement dans le navigateur, sans
                      installation. Système de points avec paiement local (CIB &
                      Edahabia).
                    </>
                  )}
                </p>
            ...
            <SlideUp delay={0.34} className="mt-12 max-w-3xl mx-auto">
              ...
              >
                ...
                >
                  ...
                  <Mono className="ms-3 text-[10px] tracking-[0.18em] uppercase text-[#16121F]/40">
                    sawtify · studio
                  </Mono>
                  <span
                    className="ms-auto flex items-center gap-1.5"
                    style={{
                      color: heroPlaying ? ACCENT : "rgba(22,18,31,0.35)",
                    }}
                  >
                    {heroPlaying && (
                      <span
                ...
                <div className="p-5 sm:p-6">
                  <div className="flex items-center justify-between gap-3 mb-4">
                    <div className="min-w-0">
                      <div className="flex items-center gap-2">
                        <Mono
                          dir="ltr"
                          className="text-[10px] font-semibold tracking-[0.14em] px-1.5 py-0.5 rounded border shrink-0"
                          style={{
                            color: featured.color,
                            borderColor: `${featured.color}55`,
                            background: `${featured.color}12`,
                          }}
                        >
                          CH{" "}
                          {String(
                            LANDING_VOICES.findIndex(
                              (v) => v.id === featured.id,
                            ) + 1,
                          ).padStart(2, "0")}
                        </Mono>
                    ...
                    <div className="text-end shrink-0">
                      <Mono className="block text-[10px] tracking-[0.2em] uppercase text-[#16121F]/40">
                        24 kHz
                      </Mono>
                      <div
                  ...
                  <div dir="ltr" className="mt-4 flex items-center gap-3">
                    <Mono className="text-[11px] text-[#16121F]/50 w-10 shrink-0 tabular-nums">
                      {fmtTime(dispElapsed)}
                    </Mono>
                    <div
                      ...
                      <button
                        ...
                        onClick={() => {
                          if (heroSamplePlaying) stopSample();
                            ...
                            stopIntroAudio();
                            playSample(featured.id, featured.audioUrl);
                          }
                        }}
                        aria-label={
                          heroSamplePlaying ? t.pause : t.listenInStudio
                        }
                        ...
                        style={{
                          background: featured.color,
                          boxShadow: `0 12px 30px -8px ${featured.color}`,
                        }}
                      >
                        {heroSamplePlaying ? (
            ...
            <SlideUp delay={0.44} className="mt-8">
              <div className="flex items-center justify-center gap-4 sm:gap-5 text-[12px] text-[#16121F]/50 flex-wrap">
                ...
                <span className="font-bold text-[#16121F]/75">
                  <Num>9</Num> {isRTL ? "أصوات" : "voix"}
                </span>
                <span>·</span>
                <span>
                  <Num>1 200+</Num> {t.creators}
                </span>
                <span>·</span>
        ...
        <section
          className="py-7"
          aria-label={isRTL ? "وسائل الدفع والجودة" : "Paiement et qualité"}
        >
          <div className="mx-auto max-w-[1280px] px-5 sm:px-6">
            <div
              className="grid grid-cols-2 sm:grid-cols-5 gap-4 rounded-2xl border bg-white px-6 py-6"
              style={{ borderColor: BORDER }}
            >
              {trust.map((p) => (
                <div key={p.k} className="text-center">
                  ...
                  <Mono className="block text-[10px] uppercase tracking-[0.14em] text-[#16121F]/45 mt-1">
                    {p.v}
                  </Mono>
                </div>
              ))}
        ...
        <section id="voices" className="py-16 sm:py-24">
          <div className="mx-auto max-w-[1280px] px-5 sm:px-6">
            ...
            <SlideUp delay={0.1}>
              <div
                className="mt-10 rounded-2xl border bg-white overflow-hidden shadow-[0_18px_50px_-30px_rgba(124,58,237,0.3)]"
                style={{ borderColor: BORDER }}
              >
                <div
                ...
                {LANDING_VOICES.map((v) => {
                  ...
                  const no = String(
                    LANDING_VOICES.findIndex((x) => x.id === v.id) + 1,
                  ).padStart(2, "0");
                  return (
                    <button
                      key={v.id}
                      ...
                      style={{
                        borderBottom: `1px solid ${BORDER}`,
                        background: active ? `${v.color}0F` : undefined,
                      }}
                    >
                      <span className="relative w-8 h-8 shrink-0 flex items-center justify-center">
                        <Mono
                          className={`absolute text-[13px] transition-opacity ${active ? "opacity-0" : "group-hover:opacity-0"}`}
                          style={{ color: "rgba(22,18,31,0.4)" }}
                        >
                          {no}
                        </Mono>
                        <span
                        ...
                        >
                          {active ? (
                            ...
                            <Play
                              className="w-4 h-4 fill-current ms-0.5"
                              style={{ color: v.color }}
                            />
                          )}
                        </span>
                      ...
                      <span className="flex-1 min-w-0">
                        <span
                          className={`block text-[15px] font-bold truncate transition-colors ${active ? "" : "group-hover:text-[#7C3AED]"}`}
                        >
                          {name}
                        </span>
                      ...
                      <span className="hidden md:flex items-center gap-1 text-[12px] font-bold text-[#16121F]/60 shrink-0">
                        <Star
                          className="w-3 h-3"
                          style={{ color: v.color, fill: v.color }}
                        />
                        <Num>{v.rating}</Num>
                        <span className="text-[#16121F]/35 font-medium ms-1">
                          (<Num>{v.reviews}</Num>)
                        </span>
                      </span>
                      <Mono
                        dir="ltr"
                        className="hidden sm:block w-16 text-end text-[12px] text-[#16121F]/45 shrink-0"
                      >
                        {durations[v.id]
                          ? fmtTime(durations[v.id])
                          : fmtDur(v, language)}
                      </Mono>
                      <span
                        className="w-10 h-10 rounded-full flex items-center justify-center text-white shrink-0 transition"
                        style={{
                          background: active ? v.color : "rgba(22,18,31,0.78)",
                          boxShadow: active
                            ? `0 8px 20px -8px ${v.color}`
                            : undefined,
                        }}
                      >
                        {active ? (
                  ...
                  {HIDDEN_VOICES.map((v) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span
                          className="w-10 h-10 rounded-full flex items-center justify-center font-bold text-[15px] shrink-0 select-none"
                          style={{
                        ...
                        <span className="flex-1 min-w-0">
                          <span
                            className="block text-[15px] font-bold truncate select-none"
                          ...
                          <span className="block text-[12px] text-[#16121F]/40 truncate">
                            {isRTL
                              ? "مقفلة — اسمعها في الاستوديو"
                              : "Verrouillée — dans le studio"}
                          </span>
                        </span>
                        <Mono
                          dir="ltr"
                          className="hidden sm:block w-16 text-end text-[12px] text-[#16121F]/30 shrink-0"
                        >
                          --:--
                        </Mono>
                        <span
                          className="w-10 h-10 rounded-full flex items-center justify-center shrink-0 transition group-hover:scale-105"
            ...
            <div className="mt-10 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {journeySteps.map((s, i) => (
                <SlideUp key={s.n} delay={i * 0.07}>
                  <div
                    className="rounded-2xl border bg-white p-6 sm:p-7 h-full min-h-[190px] flex flex-col justify-between card-lift"
                    style={{ borderColor: BORDER }}
                  >
                    <div
                    ...
                    <div>
                      ...
                      <p className="text-[12.5px] text-[#16121F]/55 leading-relaxed">
                        {s.d}
                      </p>
                    </div>
                  </div>
            ...
            <div className="mt-10 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {uses.map((u, i) => (
                <SlideUp key={u.t} delay={i * 0.06}>
                  <div
                    className="rounded-2xl border bg-white p-6 h-full card-lift"
                    style={{ borderColor: BORDER }}
                  >
                    <div
            ...
            <div className="lg:col-span-5">
              <SlideUp>
                ...
                <ul className="mt-6 space-y-2.5 text-[13px] text-[#16121F]/70">
                  {[
                    isRTL
                      ? "50 نقطة مجاناً عند التسجيل."
                      : "50 points offerts à l'inscription.",
                    isRTL
                      ? "النقاط بلا تاريخ انتهاء."
                      : "Points valables à vie.",
                    isRTL
                      ? "الدفع بالدينار عبر SATIM."
                      : "Paiement en DZD via SATIM.",
                  ].map((line) => (
                  ...
                  <div className="flex items-end justify-between gap-4 flex-wrap">
                    <div>
                      ...
                      >
                        ...
                        <span className="text-[15px] font-semibold text-[#16121F]/40 ms-2">
                          {t.pts}
                        </span>
                      </div>
                    </div>
            ...
            <div className="mt-10 grid grid-cols-2 lg:grid-cols-4 gap-4">
              {metrics.map((m, i) => (
                <SlideUp key={m.l} delay={i * 0.06}>
                  <div
                    className="rounded-2xl border bg-white p-8 text-center card-lift"
                    style={{ borderColor: BORDER }}
                  >
                    <div
                      className="text-[clamp(2.2rem,4.2vw,3rem)] leading-none font-bold mb-2"
                      style={{ color: ACCENT, fontFamily: NUM_STACK }}
                    >
                      <Counter target={m.n} suffix={m.s} />
                    </div>
                    <Mono className="text-[10px] uppercase tracking-[0.2em] text-[#16121F]/45">
                      {m.l}
                    </Mono>
                  </div>
                </SlideUp>
        ...
        <section className="py-16 sm:py-24">
          <div className="mx-auto max-w-[1280px] px-5 sm:px-6">
            ...
            <SlideUp delay={0.1}>
              <div
                className="mt-12 max-w-3xl mx-auto rounded-2xl border bg-white p-8 sm:p-12"
                style={{ borderColor: BORDER }}
              >
                <AnimatePresence mode="wait">
                  <motion.div
                  ...
                  >
                    ...
                    <blockquote
                      className="text-[clamp(1.25rem,2.6vw,1.8rem)] leading-[1.35] font-semibold"
                      style={{ fontFamily: display }}
                    >
                    ...
                    <div className="mt-7 flex items-center justify-center gap-3">
                      <div
                        ...
                        style={{
                          background: `linear-gradient(135deg, ${ACCENT}, ${MESH.pink})`,
                        }}
                      >
                      ...
                      <div className="text-start">
                        ...
                        <div className="text-[12px] text-[#16121F]/50">
                  ...
                  <div className="flex items-center gap-2">
                    {testimonials.map((_, i) => (
                      <button
                        ...
                        style={
                          i === activeTesti ? { background: ACCENT } : undefined
                        }
                      />
                    ))}
                  </div>
              ...
              <div className="mb-10 mt-10 max-w-2xl mx-auto">
                <motion.button
                  ...
                  style={{
                    ...
                    background:
                      "linear-gradient(115deg, #F6F1FF 0%, #EFE6FF 45%, #FDEFF7 100%)",
                  }}
                ...
                >
                  ...
                  <motion.div
                    aria-hidden
                    className="absolute -top-12 end-8 w-36 h-36 rounded-full pointer-events-none"
                    style={{
                  ...
                  <motion.div
                    aria-hidden
                    className="absolute -bottom-14 start-10 w-32 h-32 rounded-full pointer-events-none"
                    style={{
                  ...
                  <span className="relative flex items-center gap-3.5 px-4 sm:px-5 py-4">
                    ...
                    <span className="flex-1 min-w-0">
                      ...
                      <span className="block text-[12px] text-[#16121F]/60 mt-0.5">
                        {t.bannerSub}
                      </span>
                    </span>
                    <motion.span
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {pricing.map((p, i) => (
                <SlideUp key={p.pts} delay={i * 0.06}>
                  <div
                    className={`relative rounded-2xl border p-6 h-full flex flex-col card-lift ${p.featured ? "text-white" : "bg-white"}`}
                    style={
                      p.featured
                        ? {
                            ...
                            boxShadow:
                              "0 24px 52px -20px rgba(124,58,237,0.55)",
                          }
                        : { borderColor: BORDER }
                    }
                  >
                    {p.featured && (
                      <div
                        className="absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-1 rounded-full text-[10px] font-bold tracking-[0.12em] uppercase whitespace-nowrap"
                    ...
                    <div
                      className="text-[42px] leading-none font-bold mb-1"
                      style={{ fontFamily: NUM_STACK }}
                    >
                      <Num>{p.ptsLabel}</Num>
                    </div>
                    <Mono
                      className="block text-[10px] uppercase tracking-[0.2em] mb-4"
                      style={{
                        color: p.featured
                    ...
                    />
                    <p
                      className={`text-[12px] mb-5 flex-1 leading-relaxed ${p.featured ? "text-white/85" : "text-[#16121F]/60"}`}
                    >
                      {p.desc}
                    </p>
                    <div className="flex items-baseline gap-1.5 mb-5">
                      ...
                      <span
                        className={`text-[11px] font-semibold ${p.featured ? "text-white/70" : "text-[#16121F]/40"}`}
                      >
                        DZD
                      </span>
                    </div>
                    <button
                      ...
                      style={
                        p.featured ? { color: INK } : { background: ACCENT }
                      }
                    >
                      {t.choose}
                    </button>
                      ...
                      return (
                        ...
                        >
                          <button
                            ...
                            className="w-full py-5 px-5 sm:px-6 flex items-center gap-4 text-start focus-ring group"
                            aria-expanded={open}
                          >
                            <span className="flex-1 text-[14.5px] font-semibold group-hover:text-[#7C3AED] transition-colors">
                              {f.q}
                            </span>
                            <span
                              className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 transition-all ${open ? "rotate-45 text-white" : "text-[#16121F]/60"}`}
                              style={
                                open
                                  ? { background: ACCENT }
                                  : { background: ACCENT_SOFT }
                          ...
                          <AnimatePresence initial={false}>
                            {open && (
                              ...
                              >
                                <p className="pb-5 px-5 sm:px-6 pe-14 text-[13px] text-[#16121F]/60 leading-relaxed">
                                  {f.a}
                                </p>
                              </motion.div>
                            )}
        ...
        <section className="py-16 sm:py-24">
          <div className="mx-auto max-w-[1280px] px-5 sm:px-6">
            <SlideUp>
              <div
                className="relative rounded-[28px] p-12 sm:p-16 text-center text-white overflow-hidden"
                style={{
                  background: `linear-gradient(135deg, #6D28D9 0%, ${ACCENT} 45%, #9D5CE8 100%)`,
                  boxShadow: "0 30px 70px -30px rgba(124,58,237,0.6)",
                }}
              >
                <div
                  className="absolute top-8 start-[12%] w-44 h-44 rounded-full blur-3xl"
                  style={{ background: "rgba(255,255,255,0.18)" }}
                  aria-hidden
                />
                <div
                  className="absolute bottom-8 end-[12%] w-52 h-52 rounded-full blur-3xl"
                  style={{ background: "rgba(236,72,153,0.25)" }}
                  aria-hidden
                />
                <div className="relative">
                  <h2
                    className="text-[clamp(2.1rem,5vw,3.8rem)] leading-[1.04] tracking-[-0.02em] font-bold"
                    style={{ fontFamily: display }}
                  >
                    {t.ctaTitle}
                  </h2>
                  ...
                  <Mono className="block mt-5 text-[10px] uppercase tracking-[0.2em] text-white/70">
                    {isRTL
                      ? "بدون بطاقة · 50 نقطة مجاناً"
                      : "Sans carte · 50 points offerts"}
                  </Mono>
                </div>
              </div>
            </SlideUp>
        ...
        <footer
          id="contact"
          className="pt-12 pb-28 sm:pb-12"
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-8 mb-10">
              ...
              <div>
                ...
                <div className="flex flex-col gap-2 text-[13px] font-semibold">
                  <a
                    href="mailto:SAWTIFYSPACE@GMAIL.COM"
                    className="text-[#16121F]/70 hover:text-[#7C3AED] transition-colors"
                  >
                    SAWTIFYSPACE@GMAIL.COM
                  </a>
                  <a
                    href="tel:+213697660969"
                    className="text-[#16121F]/70 hover:text-[#7C3AED] transition-colors"
                  >
                    +213 697 660 969
                  </a>
                  <a
                    href="https://www.instagram.com/sawtify.ai"
                    target="_blank"
                    rel="noreferrer"
                    className="text-[#16121F]/70 hover:text-[#7C3AED] transition-colors"
                  >
                    Instagram · @sawtify.ai
                  </a>
                </div>
                <div className="mt-4 flex flex-wrap gap-2">
                  {["Edahabia", "CIB", "SATIM"].map((p) => (
                    <span
                      key={p}
                      className="text-[10px] font-bold px-2 py-1 rounded-full border bg-white text-[#16121F]/60"
                      style={{ borderColor: BORDER }}
                    >
                      {p}
                    </span>
                ...
                <div className="mt-4 flex items-center gap-1.5 text-[11px] text-[#16121F]/50">
                  ...
                  />
                  {isRTL
                    ? "دفع آمن، لا نخزّن رقم البطاقة."
                    : "Paiement sécurisé, aucune carte stockée."}
                </div>
              </div>
            </div>
            <div
              className="flex flex-col sm:flex-row items-center justify-between gap-3 pt-6 text-[12px] text-[#16121F]/45"
              style={{ borderTop: `1px solid ${BORDER}` }}
            >
              <span>
                © <Num>2026</Num> Sawtify · {t.footTag}
              </span>
              <div className="flex items-center gap-4">
                <button
                  type="button"
                  onClick={() => setLegal("cgu")}
      ...
      <div
        className="sm:hidden fixed bottom-0 inset-x-0 z-40 p-3 bg-white/95 backdrop-blur-xl"
        style={{ borderTop: `1px solid ${BORDER}` }}
      >
        <button
          type="button"
          ...
          className="w-full h-12 rounded-xl font-bold text-[14px] flex items-center justify-center gap-2 text-white"
          style={{ background: ACCENT }}
        >
          {t.tryFree} · 50 {t.pts}
        </button>
          ...
          <>
            ...
            >
              <div className="flex items-start justify-between gap-3 mb-4">
                <div>
                  <Mono className="block text-[10px] font-semibold uppercase tracking-[0.2em] text-[#16121F]/40 mb-1">
                    {t.listenInStudio}
                  </Mono>
                  <h3 id="listen-title" className="text-[22px] font-bold">
                    {isRTL ? listenVoice.nameAr : listenVoice.nameFr}
                  </h3>
                  <p className="text-[12px] text-[#16121F]/50">
                    {isRTL ? listenVoice.tagAr : listenVoice.tagFr} ·{" "}
                    {listenVoice.location}
                  </p>
                </div>
                <button
                  type="button"
                  ...
                  className="w-9 h-9 rounded-full hover:bg-[#16121F]/5 flex items-center justify-center focus-ring"
                  aria-label={t.close}
                >
                  <X className="w-4 h-4" />
                </button>
              ...
              <div
                ...
                style={{
                  borderColor: BORDER,
                  background: `${listenVoice.color}0D`,
                }}
              >
                <Waveform
                  color={listenVoice.color}
                  playing={playingId === listenVoice.id}
                  bars={30}
                />
                <button
                  type="button"
                  onClick={() =>
                    playingId === listenVoice.id
                      ? stopSample()
                      : playSample(listenVoice.id, listenVoice.audioUrl)
                  }
                ...
                <p className="mt-2 text-[11px] text-center text-[#16121F]/45">
                  {t.studioNote}
                </p>
              </div>
              <p
                className="text-[13px] leading-relaxed text-[#16121F]/70 mb-2"
                dir="auto"
              >
                "
              ...
              <p className="text-[13px] text-[#16121F]/60 leading-relaxed mb-5">
                {t.listenBody}
              </p>
              <button
                type="button"
        ...
        {legal && (
          <>
            ...
            >
              ...
              <p className="text-[13px] leading-relaxed text-[#16121F]/60">
                {legalCopy[legal]}
              </p>
            </motion.div>
          </>
      ...
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            "@type": "HowTo",
            name: isRTL
              ? "كيف تحوّل النص إلى صوت بالدارجة"
              : "Comment convertir un texte en voix darija",
            description: isRTL
              ? "دليل خطوة بخطوة"
              : "Guide étape par étape pour générer une voix off en darija",
            totalTime: "PT1M",
