...
<div className={`
  ...
  z-[65] /* Forcé au-dessus de la nav (qui est souvent z-30/40) */
  w-[85vw] max-w-[360px] /* Largeur max pour ne pas être trop étroit */
  h-[100dvh] /* Hauteur totale écran : ne coupe jamais l'écran en deux */
  shrink-0 
...
`}>
  ...
  <div className="shrink-0 flex items-center justify-between p-4 border-b border-slate-100 bg-white">
    ...
    <button 
      onClick={() => setIsScriptMenuOpen(false)} 
      className="
        p-1.5 rounded-lg hover:bg-red-50 text-slate-400 
        cursor-pointer active:bg-red-100 active:scale-95
        min-h-[36px] min-w-[36px]
        flex items-center justify-center
        transition-colors
        // Pas de backdrop-blur ici pour éviter le lag mobile
      "
      aria-label={language === 'ar' ? 'إغلقق' : 'Fermer'}
    >
      <X className="w-4 h-4" />
    </button>
  </div>
  ...
  <div className="flex-1 min-h-0 overflow-hidden overscroll-contain p-4 space-y-3">
    ...
    <div className="space-y-2">
      <input 
        ...
        className="
          ...
          min-h-[44px] /* Touch target confortable */
        "
      />
      
      {/* Bouton générer script - Touche target optimum */}
      <button 
        ...
        className="
          ...
          transition-all duration-200 
          min-h-[48px] /* Très tactile friendly */
          touch-manipulation
          select-none
        "
      >
...
<div className={`
  fixed lg:static 
  inset-y-0 
  end-0  {/* end-0 en LTR, start-0 en RTL */}
  z-[70] /* Au-dessus du player audio (z-65) et de la nav (z-55) */
  w-[85vw] 
  max-w-[380px]
  h-[90dvh] /* Presque la hauteur de l'écran pour pouvoir scroller la liste de voix */
  shrink-0 
  border-l 
  border-slate-200 
  bg-white 
...
`}>
  ...
  <div className="flex-1 min-h-0 flex-col">
    ...
    {/* Filtre genre rapide - Style pill */}
    ...
    {/* Sélecteur catégorie (compact sur mobile) */}
    ...
    {/* Liste des voix - Scroll optimisé */}
    <div className="flex-1 min-h-0 overflow-hidden overscroll-contain px-2 py-2 space-y-1.5">
      {filteredVoices.map((voice) => {
        ...
        return (
          <div 
            key={voice.id}
            onClick={() => {
              // Fermer l'autre drawer si ouvert
              if (isVoiceMenuOpen && window.innerWidth < 1024) {
                // Sur mobile, fermer automatiquement le drawer de script avant d'ouvrir celui-ci
                setIsScriptMenuOpen(false);
              }
              setSelectedVoiceId(voice.id);
              setIsVoiceMenuOpen(false);
              
              // Ne pas fermer si c'est la même voix (évite le "clignotement")
              if (!isSelected) {
                setIsPlayerMinimized(true); // Minimiser le player si changement
              }
            }}
            ...
            className={`
              ...
              ${
                isSelected
                  ? 'bg-purple-50 border-purple-400/60 shadow-sm'
                  'border-purple-300'
                  'shadow-md'
            ...
            <div className="flex items-center justify-between mt-1.5">
              <div className="flex items-center gap-1.5">
                <button
                  ...
                  `
                `}
                  title={favoriteVoiceIds.includes(voice.id) ? (language === 'ar' ? 'إزالة من المفضلة' : 'Retirer des favoris') : (language === 'ajouter aux favoris')}
                >
                  <Star
                    className={`w-4 h-4 ${favoriteVoiceIds.includes(voice.id) ? 'fill-amber-500' : ''}`}
                    transition-colors
    ...
    {/* Sliders settings bas de la voix sélectionnée - toujours visibles même en scrollant */}
    <div className="shrink-0 px-4 py-3 bg-slate-50 rounded-xl border-t border-slate-200/80">
      <div className="flex items-center gap-2 mb-3">
        <div className={`w-9 h-9 rounded-xl ${previewingVoiceId === currentVoiceId ? 'bg-purple-600 text-white shadow-md scale-95' : 'bg-white text-slate-500 border border-slate-200'} flex items-center justify-center`}>
        ...
        <div className="min-w-0">
          ...
          <p className="text-[10px] text-slate-400 truncate mt-0.5">{
            `${language === 'ar' ? 'النوع الصوت:' : 'Type de voix:'} 
          } {currentVoice.tags?.join(', ') || (language === 'ar' ? 'عربي' : 'Standard')}</p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <div className="flex items-center justify-between text-[10px] text-slate-700 bg-white border border-slate-200 rounded-xl px-3 py-2 hover:border-purple-400/50">
