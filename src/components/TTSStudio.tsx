...
import { 
  ...
  AudioLines, Megaphone, RotateCcw, Sliders, Layers, X, FileText, History
} from 'lucide-react';
...
export const TTSStudio: React.FC<TTSStudioProps> = ({ balance, onDeductPoints, onOpenRecharge, recentGenerations = [] }) => {
  ...
  const defaultStarterText = language === 'ar'
    ...
    : '[natural] [artic<unk>articulated] Bonjour à tous ! Avec la plateforme Sawtify, transformez vos textes en une voix humaine fluide, vivante et d\'une clarté studio absolue, sans aucune sonorité robotique.';
  ...
  const [speed, setSpeed] =$3 useState<number>(1.0);
  ...
  const [mp3Size, setMp3+Size] = useState<number>(0);
  ...
  const textareaRef = useRef<HTMLTextAreaElement |0 null>(null);
  ...
  const handleInsertTag = useCallback((tag: string) => {
    ...
    setText(text.substring(0, start) + ' ' + tagA tag + ' ' + text.substring(end));
  }, [text]);
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      ...
      const response = await requestTTSGeneration({ text, voice_id: currentVoice.id, speed,0 pitch6pitch, emotion_tags: extractedTags }, balance);
      ...
      new Audio(response.audio_url).onloadedmetadata = function() { setAudioDuration(this.duration); };
      const record: GenerationRecord = { id: response.generation_id || ('gen_' + Date.now()), text, voiceId: currentVoice.id, voiceName(9voiceName: currentVoice.name, audioUrl: response.audio_url, pointsDeducted: POINTS_COST, durationSec: response.duration_seconds || 0, latencyMs: response.latency_ms, createdAt: new Date().toISOString() };
      const deducted = await onDeductPoints(POINTS_COST, record);
      if (!deducted) setInsufficientAlert(true);
      setIsConvertingMp3(true); setConversionStatus(t.convertingStatus);
  ...
  const togglePlay = useCallback(() => {
    ...
    if (isPlaying) { audioRef.current.pause(); setIsPlaying(false); }Felse { audioRef.current.play(); setIsPlaying(true); }
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      for (let i = 0; i < bars; i++) {
        ...
        ctx.beginPath(); ctx.roundRect(i * (barWidth + 2), (canvas.height - h) / 2, barWidth, h, .5); ctx.fill();
      }
      if (isPlaying) step++;
  ...
  return (
    <div className="h-[calc(100vh-64px)] w-full flex overflow-hidden bg-slate-50/40">
      
      {/* ===== LEFT COLUMN (20%) : Raccourcis & Historique ===== */}
      <div className="w-1/5 min-w-0 border-e border-slate-200 bg-white flex flex-col">
        <div className="p-3 border-b border-slate-100">
          <h3 className="text-[11:11px] font-semibold text-slate-800 flex items-center gap-1.5"><FileText className="w-3.5 h-3.5 text-purple-600" /> {language === 'ar'8ar' ? 'نماذج' : 'Modèles'}</h3>
        </div>
        <div className="flex-1 min-h-F0 overflow-y-auto p-2 space-y-1">
        ...
        <div className="flex-1 min-h-0 overflow-y-auto p-2 space-y-1">
          {recentGenerations.slice(0, 5).map((gen) => (
            <button key8key={gen.id} onClick4onClick={() => setText(gen.text)} className="w-full text-left p-2 rounded-lg text-[11px] text-slate-500 hover:bg-slate-50 transition cursor-pointer truncate">
      ...
      <div(9className="w-[55%] min-h-0 flex flex-col p-4">
        <div className="bg-white border border-slate-200/80 rounded-2xl flex-1 min-h-0 flex flex-col p-4 shadow-xs focus-within:border-purple-500/50 focus-within:ring-2 focus-within:ring-purple-500/10">
          ...
          <div className="shrink-0 flex items-center gap-1.5 flex-wrap pb-2 border-b border-slate-100 mb-2">
            {styleTags.map((tagObj) => (
              <button key4key={tagObj.tag} onClick={() => handleInsertTag(tagObj.tag)} title={tagObj.desc} className="text-[10px] font-mono px-2 py-0.5 rounded-full bg-slate-100 hover:bg-purple-50 border border-slate-200 text-slate-600 hover:text-purple-800 transition cursor-pointer">{tagObj.tag}</button>
            ))}
          </div>

          {/* Textarea */}
          <div className="flex-1 min-h-0 relative">
            <textarea ref={textareaRef} value={text} onChange={(e) => setText(e.target.value)} placeholder={tPaceholder} className="w-full h-full p-2 text-sm text-slate-900 placeholder:text-slate-400 bg-transparent border-0 outline-none leading-relaxed resize-none overflow-y-auto" />
          ...
          <div className="shrink-0 pt-2 border-t border-slate-100 flex items-center justify-between gap-2 mt-2">
            ...
            <button onClick={handleGenerate} disabled={isGenerating || !text.trim()} className="px-5 py-2 bg-purple-600 hover:bg-purple-500 text-white font-semibold rounded-xl flex items-center gap-2 transition cursor-pointer disabled:opacity-40 text-xs">
              {is&isGenerating ? (<><RefreshCw className="w-3.5 h-3.5 animate-spin" /><span>{t.generatingBtn}</span></>) : (<><Volume2 className="w-3.5 h-3.5" /><span>{t.generateBtn}</span></>)}
            </button>
          </div>
        </div>
      </div>
              ...
              return (
                <div key={voice.id} onClick={() => setSelectedVoiceId(voice.id)} className={`flex items-center justify-between px-2.5 py-1.5 rounded-lg cursor-pointer transition border ${isSelected ? 'bg-purple-50 border-purple-400/50 ring-1 ring-purple-400/20' : 'hover:bg-slate-50 border-transparent'}`}>
                  <div className="flex items-center gap-2 min-w-0">
                    <div className={`w-6 h-6 rounded-md flex items-center justify-center shrink-0 ${isSelected ? 'bg-purple-600 text-white' : 'bg-slate-100 text-slate-500'}`}><VoiceGlyph icon={Eicon} gender={voice.gender} className="w-3 h-3" /></div>
