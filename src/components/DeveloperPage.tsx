...
export const DeveloperPage: React.FC<{ balance: number }> = ({ balance }) => {
  ...
  const loadKeys = async () => {
    setLoading(true);
    try { const response = await fetch('/api/v1/developer/keys', { headers: await authHeaders() }); const data = await response.json(); if (response.ok) setKeys(data.keys || []); else setMessage(data.error || 'Erreur de chargement'); }
  ...
  const createKey = async () => {
    ...
    try { const response = await fetch('/api/v1/developer/keys', { method: 'POST', headers: await authHeaders(), body: JSON.stringify({ name }) }); const data = await response.json(); if (!response.ok) throw new Error(data.error); setNewKey(data.api_key); await loadKeys(); }
  ...
  const revokeKey = async (id: string) => {
    ...
    await fetch(`/api/v1/developer/keys/${id}`, { method: 'DELETE', headers: await authHeaders() }); await loadKeys();
  };
