...
async function synthesizeWithRetry(rawText: string, selectedVoiceName: string, maxRetries = 3, speed = 1.0, pitch = 1.0, originalVoiceId: string = ""): Promise<{ pcmBuffer: Buffer | null; error: string | null }> {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (pcmBase64 && pcmBase64.length > 50) { console.log(`[TTS] Succès Gemini TTS: ${pcmBase64.length} caractères base64 reçus.`); return { pcmBuffer: Buffer.from(pcmBase64, "base64"), error: null }; }
      console.warn(`[TTS Attempt ${attempt}] Aucun PCM audio renvoyé.`);
    } catch (err: any) { lastError = err; console.warn(`[TTS Attempt ${attempt}/${maxRetries}] Erreur Gemini:`, err.message || err); if (attempt < maxRetries) { const delay = 400 * Math.pow(2, attempt - 1) + Math.random() * 150; await new Promise((resolve) => setTimeout(resolve, delay)); } }
  }
...
async function startServer() {
  ...
  app.post("/api/slickpay/create-invoice", async (req, res) => {
    try {
      ...
      const callConfigs = isDevConfigured ? [ { url: "https://devapi.slick-pay.com/api/v2/users/invoices", key: SLICKPAY_KEY, payload: payloadC, desc: "DevAPI" }, { url: "https://devapi.slick-pay.com/api/v2/users/invoices", key: "54|BZ7F6N4KwSD46GEXToOv3ZBpJpf7WVxnBzK5cOE6", payload: payloadC, desc: "Sandbox" }, { url: "https://prodapi.slick-pay.com/api/v2/users/invoices", key: SLICKPAY_KEY, payload: payloadA, desc: "Prod" } ] : [ { url: primaryUrl, key: SLICKPAY_KEY, payload: payloadA, desc: "Prod" }, { url: "https://devapi.slick-pay.com/api/v2/users/invoices", key: SLICKPAY_KEY, payload: payloadC, desc: "DevAPI" }, { url: "https://devapi.slick-pay.com/api/v2/users/invoices", key: "54|BZ7F6N4KwSD46GEXToOv3ZBpJpf7WVxnBzK5cOE6", payload: payloadC, desc: "Sandbox" } ];
